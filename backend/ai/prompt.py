...
#: ``{user_message}`` y llaves dobles ``{{`` / ``}}`` para el JSON literal, de
#: modo que se resuelve con :meth:`str.format`.
...
def build_prompt(user_message: str, ctx: PromptContext) -> str:
    ...
    Función pura: el resultado depende únicamente de ``user_message`` y ``ctx``.
